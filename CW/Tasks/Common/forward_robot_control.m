...
i = 0;
    ...
    while (j<200)
        ...
        deg3 = dxl_present_position3* encoder_to_degree;
        deg4 = dxl_present_position4* encoder_to_degree;
        deg5 = dxl_present_position5* encoder_to_degree;
        ...
        if ~(abs(dxl_goal_position(index) - typecast(int32(dxl_present_position1), 'int32')) > DXL_MOVING_STATUS_THRESHOLD)
            break;
        end
        
        fprintf("deg 5: %f\n", deg5);

%         fprintf('[ID:%03d] Position5: %03d\n', DXL_ID5, typecast(uint32(deg5), 'int32'));
