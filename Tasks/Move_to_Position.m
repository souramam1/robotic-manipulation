function [] = Move_to_Position(pose, x, y, z)
...
for j = 1:size(x) + 1
    ...
    [x_des] = cubic_trajectory(x_cur, x(j), time, steps);
    [y_des] = cubic_trajectory(y_cur, y(j), time, steps);
    [z_des] = cubic_trajectory(z_cur, z(j), time, steps);
    ...
    for i = 1:steps
        ...
        [theta1_des, alpha2_des, alpha3_des, alpha4_des] = inverse_kinematics(x_des(i), y_des(i), z_des(i), pose(j));
        ...
        [pose_read,x_read,y_read,z_read] = forward_kinematics(deg1, deg2, deg3,deg4);
